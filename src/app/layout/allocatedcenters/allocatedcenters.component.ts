...
export class AllocatedcentersComponent implements OnInit {
	...
	pageLoadProcess(){
		...
		this.allocatedcentersService.allocatedcenter_getalldetails().subscribe(data => {
				//console.log('--> all_data: '+JSON.stringify(data));
				if(Object.keys(data).length > 0){
					let result_array:any = [];
					result_array = data;
					this.all_data = result_array.filter(element => (Object.keys(element._id).length > 0 && element._id.managerid.length > 0) );
					this.filterData = this.all_data;
					//console.log('--> all_data: '+JSON.stringify(this.all_data));
				}
				this.hideLoading_indicator = true;
			}, error => {}, () => {}
		);
	...
	search(term: string) {
		if(!term) {
		  ...
		  this.filterData = this.all_data.filter(element => 
			element._id.managername.trim().toLowerCase().includes(term.trim().toLowerCase())
		  );
		}
	}
