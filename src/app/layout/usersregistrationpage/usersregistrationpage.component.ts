...
export class UsersregistrationpageComponent implements OnInit {
  ...
  // Usertype
  all_available_usertypes_list: any = [
    { typeid: "anganwadi", typetext: "Anganwadi" },
    { typeid: "school", typetext: "School" },
    { typeid: "fellow", typetext: "Fellow" },
    { typeid: "manager", typetext: "Manager" },
  ];
  ...
  get_availavle_centers() {
    ...
    this.usersregistrationpageService.userreg_getavailablecenters().subscribe(
      (data) => {
  ...
  get_center_by_centerid(centerid) {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService
      .userreg_getcenterbycenterid(centerid)
      .subscribe(
        (data) => {
          if (Object.keys(data).length > 0) {
            this.selected_statecode =
              data[0].statecode == undefined ||
              data[0].statecode == null ||
              data[0].statecode.trim() == ""
                ? ""
                : data[0].statecode;
            this.selected_statevalue =
              data[0].statevalue == undefined ||
              data[0].statevalue == null ||
              data[0].statevalue.trim() == ""
                ? ""
                : data[0].statevalue;
            this.selected_districtid =
              data[0].districtid == undefined ||
              data[0].districtid == null ||
              data[0].districtid.trim() == ""
                ? ""
                : data[0].districtid;
            this.selected_districtvalue =
              data[0].districtvalue == undefined ||
              data[0].districtvalue == null ||
              data[0].districtvalue.trim() == ""
                ? ""
                : data[0].districtvalue;
            this.selected_block =
              data[0].block == undefined ||
              data[0].block == null ||
              data[0].block.trim() == ""
                ? ""
                : data[0].block;
  ...
  get_availavle_managers() {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService.userreg_getmanagers().subscribe(
      (data) => {
        if (Object.keys(data).length > 0) {
  ...
  get_availavle_states() {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService.userreg_getstates().subscribe(
      (data) => {
        if (Object.keys(data).length > 0) {
  ...
  get_availavle_districts(statecode) {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService.userreg_getdistricts(statecode).subscribe(
      (data) => {
        if (Object.keys(data).length > 0) {
  ...
  update_center_status(id, body) {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService.userreg_updatecenter(id, body).subscribe(
      (data) => {
        this.hideLoading_indicator = true;
      },
      (error) => {},
  ...
  release_center_button_click() {
    swal
      ...
      .then((result) => {
        if (result.value) {
          if (
            this.selected_centerid != undefined &&
            this.selected_centerid != null &&
            this.selected_centerid.length > 0
          ) {
            // update center status
            this.update_center_status(this.selected_centerid, {
              status: "available",
            });
            // update user
            let body = {
            ...
            this.hideLoading_indicator = false;
            this.usersregistrationpageService
              .userreg_updateuser(this.document_id, body)
              .subscribe(
                (data) => {
                  this.hideLoading_indicator = true;
                },
  ...
  get_userbyuserid() {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService
      .userreg_getuserbyuserid(this.qp_userid)
      .subscribe(
        (data) => {
          if (Object.keys(data).length > 0) {
            let user = data[0];
            ...
            this.selected_usertypeid =
              this.selected_usertypeid == undefined ||
              this.selected_usertypeid == null ||
              this.selected_usertypeid.trim() == ""
                ? ""
                : this.selected_usertypeid.trim().toLowerCase();
            this.selected_gender =
              this.selected_gender == undefined ||
              this.selected_gender == null ||
              this.selected_gender.trim() == ""
                ? ""
                : this.selected_gender.trim().toLowerCase();

            if (
              this.selected_usertypeid.trim().toLowerCase() == "anganwadi" ||
              this.selected_usertypeid.trim().toLowerCase() == "school" ||
              this.selected_usertypeid.trim().toLowerCase() == "fellow"
            ) {
              ...
              this.selected_usertypeid.trim().toLowerCase() == "manager"
            ) {
  ...
  set_selected_list() {
    if (this.selected_usertypeid.trim().length > 0) {
      this.disable_usertype_dropdown =
        this.usersubmitaction == "Update" ? true : false;
      if (this.selected_usertypeid.trim().toLowerCase() == "anganwadi")
        this.selected_usertype_list = [
          { typeid: "anganwadi", typetext: "Anganwadi" },
        ];
      else if (this.selected_usertypeid.trim().toLowerCase() == "school")
        this.selected_usertype_list = [
          { typeid: "school", typetext: "School" },
        ];
      else if (this.selected_usertypeid.trim().toLowerCase() == "fellow")
        this.selected_usertype_list = [
          { typeid: "fellow", typetext: "Fellow" },
        ];
      ...
      this.selected_center_list = [
        {
          centerid: this.selected_centerid,
          centername: this.selected_centername,
        },
      ];
      this.selected_manager_list = [
        {
          userid: this.selected_managerid,
          username: this.selected_managername,
        },
      ];
      if (this.selected_gender.trim().toLowerCase() == "male")
        this.selected_gender_list = [
          { genderid: this.selected_gender, gendername: "Male" },
        ];
      else if (this.selected_gender.trim().toLowerCase() == "female")
        this.selected_gender_list = [
          { genderid: this.selected_gender, gendername: "Female" },
        ];
      else
        this.selected_gender_list = [
          { genderid: this.selected_gender, gendername: "Other" },
        ];
      this.selected_state_list = [
        { _id: this.selected_statecode, statevalue: this.selected_statevalue },
      ];
      this.selected_district_list = [
        {
          _id: this.selected_districtid,
  ...
  usertype_multiselect_onselect(item: any) {
    ...
    this.selected_usertype_list = [
      { typeid: item.typeid, typetext: item.typetext },
    ];
    if (
      this.selected_usertypeid.trim().toLowerCase() == "anganwadi" ||
      this.selected_usertypeid.trim().toLowerCase() == "school" ||
  ...
  center_multiselect_onselect(item: any) {
    ...
    this.selected_centerid =
      this.selected_centerid == undefined ||
      this.selected_centerid == null ||
      this.selected_centerid.trim() == ""
        ? ""
        : this.selected_centerid;
    this.selected_centername =
      this.selected_centername == undefined ||
      this.selected_centername == null ||
      this.selected_centername.trim() == ""
        ? ""
        : this.selected_centername;
  ...
  manager_multiselect_onselect(item: any) {
    ...
    this.selected_managerid =
      this.selected_managerid == undefined ||
      this.selected_managerid == null ||
      this.selected_managerid.trim() == ""
        ? ""
        : this.selected_managerid;
    this.selected_managername =
      this.selected_managername == undefined ||
      this.selected_managername == null ||
      this.selected_managername.trim() == ""
        ? ""
        : this.selected_managername;
  }
  ...
  state_multiselect_onselect(item: any) {
    ...
    this.selected_statecode =
      this.selected_statecode == undefined ||
      this.selected_statecode == null ||
      this.selected_statecode.trim() == ""
        ? ""
        : this.selected_statecode;
    this.selected_statevalue =
      this.selected_statevalue == undefined ||
      this.selected_statevalue == null ||
      this.selected_statevalue.trim() == ""
        ? ""
        : this.selected_statevalue;
  ...
  formSubmitAction(usersubmitaction) {
    ...
    this.selected_username =
      ...
      this.selected_username.trim() == ""
        ? ""
        : this.selected_username.toLowerCase();
    this.selected_usertypeid =
      this.selected_usertypeid == undefined ||
      this.selected_usertypeid == null ||
      this.selected_usertypeid.trim() == ""
        ? ""
        : this.selected_usertypeid.toLowerCase();
    this.selected_password =
      ...
      this.selected_password.trim() == ""
        ...
        : this.selected_password;
    this.selected_email =
      this.selected_email == undefined ||
      this.selected_email == null ||
      this.selected_email.trim() == ""
        ? ""
        : this.selected_email.toLowerCase();
    this.selected_centerid =
      this.selected_centerid == undefined ||
      this.selected_centerid == null ||
      this.selected_centerid.trim() == ""
    ...
    this.selected_districtid =
      ...
      this.selected_districtid == null ||
      this.selected_districtid.trim() == ""
        ? ""
        : this.selected_districtid;
    this.selected_districtvalue =
      this.selected_districtvalue == undefined ||
      this.selected_districtvalue == null ||
      this.selected_districtvalue.trim() == ""
        ? ""
        : this.selected_districtvalue;
    this.selected_block =
      this.selected_block == undefined ||
      this.selected_block == null ||
      this.selected_block.trim() == ""
        ? ""
        : this.selected_block.toLowerCase();
    this.selected_phone =
    ...
    this.selected_gender =
      ...
      this.selected_gender.trim() == ""
        ? ""
        : this.selected_gender.toLowerCase();
    this.selected_permanentaddress =
      this.selected_permanentaddress == undefined ||
      this.selected_permanentaddress == null ||
      this.selected_permanentaddress.trim() == ""
        ? ""
        : this.selected_permanentaddress.toLowerCase();
    this.profileimage =
      this.image_s3url == undefined ||
      this.image_s3url == null ||
      this.image_s3url.trim() == ""
        ? this.profileimage
        : this.image_s3url;
    ...
    if (usertype.trim() == "") {
      ...
      if (username.trim() == "") {
        ...
        swal.fire(
          "Info",
          "Password must be of 5 or more character length.",
          "warning"
        );
      } else if (
        emailid.trim() != "" &&
        !this.selected_email.match(this.validatemail)
      ) {
        ...
        if (centerid.trim() == "") {
          ...
          swal.fire(
            "Info",
            "Contact number is not in valid format. Valid formats: 0123456789, (012)345-6789, 012-345-6789",
            "warning"
          );
          ...
          swal.fire(
            "Info",
            "Invalid district.(check Center master)",
            "warning"
          );
        } else if (block.trim() == "") {
          swal.fire(
            "Info",
            "Invalid block name.(check Center master)",
            "warning"
          );
        } else if (block.trim().length < 4) {
          swal.fire(
            "Info",
            "Block name must be of 4 or more character length.",
            "warning"
          );
        } else if (
  ...
  saveuser(body) {
    ...
    this.usersregistrationpageService.userreg_createuser(body).subscribe(
      (data) => {
        swal.fire(
          "Success",
          "User profile registered successfully.",
          "success"
        );
        this.reset_all();
        this.hideLoading_indicator = true;
  ...
  updateuser(id, body) {
    this.hideLoading_indicator = false;
    this.usersregistrationpageService.userreg_updateuser(id, body).subscribe(
      (data) => {
  ...
  public get imageToDisplay() {
    if (this.currentImage) {
      this.profileimage = this.currentImage;
  ...
  save_to_s3() {
    ...
    this.managersboxService
      .pushFileToStorage(imageFile, this.image_filename_s3)
      .subscribe((event) => {
  ...
  savetodb() {
    ...
    this.managersboxService.uploadToManagersBox(body).subscribe(
      (data) => {
        this.hideLoading_indicator = true;
      },
      (error) => {},
