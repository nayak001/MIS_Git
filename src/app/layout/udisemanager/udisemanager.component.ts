...
export class UdisemanagerComponent implements OnInit {
  ...
  constructor(
	...
	getalludisecodes(){
		...
		this.udisemanagerService.getalludisecodes().subscribe(data => {
  ...
  save_data(){
    if(this.txt_udisecode == null || this.txt_udisecode == undefined || this.txt_udisecode == '') {
      swal.fire('Info','UDISE Code is not valid', 'warning');
    }else if(this.txt_schoolname == null || this.txt_schoolname == undefined || this.txt_schoolname == '') {
      swal.fire('Info', 'Schoolname is not valid', 'warning');
    }else{
      this.hideLoading_indicator = false;
      this.udisemanagerService.checkudisecodeexistance(this.txt_udisecode).subscribe(data1 => {
        console.log('-->save check udise: ',data1);
        if(Object.keys(data1).length > 0){
          ...
          this.udisemanagerService.saveudisecode(body).subscribe(data2 => {
            this.modalReference.close();
            swal.fire('Success', 'UDISE Record saved successfully', 'success');
            this.getalludisecodes();
            this.hideLoading_indicator = true;
            this.reset();
          },error => {},() => {});
  ...
  update_data(){
    if(this.txt_udisecode == null || this.txt_udisecode == undefined || this.txt_udisecode == '') {
      ...
      this.hideLoading_indicator = false;
      this.udisemanagerService.checkudisecodeexistance(this.txt_udisecode).subscribe(data1 => {
        console.log('-->update check udise: ',data1);
        if(Object.keys(data1).length > 0){
          if(data1[0]._id == this.record_id){
            ...
            this.udisemanagerService.updateudisecode(this.record_id, body).subscribe(data2 => {
              this.modalReference.close();
              swal.fire('Success', 'UDISE Record updated successfully', 'success');
              this.getalludisecodes();
              this.hideLoading_indicator = true;
              this.reset();
            },error => {},() => {});
  ...
  delete_data(){
    swal.fire({
      ...
      if (result.value) {
        this.hideLoading_indicator = false;
        this.udisemanagerService.deleteudisecode(this.record_id).subscribe(data => {
          swal.fire('Success', 'UDISE Record deleted successfully', 'success');
        },error => {
