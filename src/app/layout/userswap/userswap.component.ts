...
export class UserswapComponent implements OnInit {
  ...
  search(term: string) {
    ...
    if (!term) {
      ...
      else if (this.selected_searchfilter == "centername")
        this.searchcentername(term);
    }
  }
  searchusername(term) {
  ...
  navigate_to_registrationpage(users_object) {
    ...
    if (
      users_object == null ||
      users_object == undefined ||
      Object.keys(users_object).length <= 0
    ) {
      navigationExtras = {
        queryParams: {
          action: "new_user",
  ...
  set_userdetails(user) {
    ...
    this.userdetail_statecode =
      user.statecode == undefined ||
      user.statecode == null ||
      user.statecode.trim() == ""
        ? "Not Found"
        : user.statecode.trim();
    this.userdetail_statevalue =
      user.statevalue == undefined ||
      user.statevalue == null ||
      user.statevalue.trim() == ""
        ? "Not Found"
        : user.statevalue.trim().toLowerCase();
    ...
    this.userdetail_districtvalue =
      user.districtvalue == undefined ||
      user.districtvalue == null ||
      user.districtvalue.trim() == ""
        ? "Not Found"
        : user.districtvalue.trim().toLowerCase();
    this.userdetail_block =
      user.block == undefined || user.block == null || user.block.trim() == ""
        ? "Not Found"
        : user.block.trim().toLowerCase();
