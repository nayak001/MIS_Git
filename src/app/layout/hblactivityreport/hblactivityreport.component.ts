...
export class HblactivityreportComponent implements OnInit {
  ...
  getalldistricts() {
    ...
    this.hblactivityreportService.getdistincthbldistricts().subscribe(
      (data) => {
  ...
  getallmanagers() {
    this.hideLoading_indicator = false;
    this.hblactivityreportService.getdistincthblmanagers().subscribe(
      (data) => {
  ...
  toggleCloseDropdownSelection() {
    ...
    this.week_multiselect_settings = Object.assign(
      {},
      this.week_multiselect_settings,
      { closeDropDownOnSelection: this.closeDropdownSelection }
    );
  ...
  getreportdatabydistricts() {
    ...
    this.hideLoading_indicator = false;
    this.hblactivityreportService
      .getdistincthblschoolsbydistrictsarray(obj)
      .subscribe(
        (data) => {
  ...
  getreportdatabymanagers() {
    ...
    this.hideLoading_indicator = false;
    this.hblactivityreportService.gethblreportdatabymanagers(obj).subscribe(
      (data) => {
        if (Object.keys(data).length > 0) {
          this.current_report_data = data["studentdetails"];
          this.current_report_data_bkp = data["studentdetails"];
  ...
  async getreport(filtervalue) {
    if (filtervalue === "district") {
      if (
        this.district_multiselect_selectedlist == undefined ||
        this.district_multiselect_selectedlist == null ||
        this.district_multiselect_selectedlist.length <= 0
      ) {
        swal.fire("Info", "Please select some districts", "warning");
        this.current_report_data = [];
      } else if (
        this.week_multiselect_selectedlist == undefined ||
        this.week_multiselect_selectedlist == null ||
        this.week_multiselect_selectedlist.length <= 0
      ) {
      ...
      if (
        this.manager_multiselect_selectedlist == undefined ||
        this.manager_multiselect_selectedlist == null ||
        this.manager_multiselect_selectedlist.length <= 0
      ) {
  ...
  getallhblreportdata() {
    ...
    this.hblactivityreportService.getallhblreportdata().subscribe(
      (data) => {
  ...
  gethblreportdatabyschools_csv() {
    ...
    this.hideModalLoading_indicator = false;
    this.hblactivityreportService.gethblreportdatabyschools_csv(obj).subscribe(
      (data) => {
        this.all_report_data = data["studentdetails"];
        this.hideModalLoading_indicator = true;
      },
  ...
  gethblreportdatabymanagers_csv() {
    ...
    this.hideModalLoading_indicator = false;
    this.hblactivityreportService.gethblreportdatabymanagers_csv(obj).subscribe(
      (data) => {
        this.all_report_data = data["studentdetails"];
        this.hideModalLoading_indicator = true;
      },
  ...
  searchstudent(term: string) {
    ...
    if (!term) {
      ...
      this.current_report_data = this.current_report_data_bkp.filter(
        (element) =>
