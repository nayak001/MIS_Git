...
export class CenterDetailsComponent implements OnInit {
  ...
  getBlockDetails() {
    this.centerDetailsService.getBlocks().subscribe(
      (data) => {
        ...
        for (var i = 0; i < this.allDisticBlocks.length; i++) {
          if (this.allDistics.length > 0) {
            ...
            for (var j = 0; j < this.allDistics.length; j++) {
              if (
                this.allDistics[j].districtvalue ==
                this.allDisticBlocks[i].districtvalue
              ) {
                isDistic = true;
              }
            }
  ...
  download() {
    ...
    const rows = [
      [
        ...
        "PG Math Level 5",
        "Average Attendance",
        "Percentage of students achieving monthly goals in PG - English",
        "Percentage of students achieving monthly goals in PG - Odia",
        "Percentage of students achieving monthly goals in PG - Maths	",
        "Percentage of students achieving atleast 70% of age-appropriate skills in ECEProgram",
        "Percentage of students who jumped 1 Level in PG - English",
        "Percentage of students who jumped 1 Level in PG - Odia",
        "Percentage of students who jumped 1 Level in PG - Maths",
        "Percentage of students who jumped 2 Levels in PG - English",
        "Percentage of students who jumped 2 Levels in PG - Odia",
        "Percentage of students who jumped 2 Levels in PG - Maths",
        "No of Students who have graduated from ECE program	",
        "No of Students who have graduated from PG program",
        "No of Students who have dropped out of the ECE program",
        "No of Students who have dropped out of the PG program",
    ...
    this.centerDetailsService.getCenterDetails(data).subscribe((data) => {
      ...
      this.alldata.forEach((value) => {
        ...
        var array = [
          ...
          usertType,
          value.center.centertype || "",
          value.center.block || "",
          value.center.district || "",
          value.state,
          value.educator,
          value.no_of_students,
          value.no_female_students,
          value.eceLevel1,
          value.eceLevel2,
          value.eceLevel3,
          value.pgEngl1,
          ...
          value.student_dropped_pg_program,
          value.grade_of_student,
        ];
        rows.push(array);
      });
