...
export class SkillchartfileuploadComponent implements OnInit {
	...
	constructor(private skillchartfileuploadService: SkillchartfileuploadService) {
	...
	uploadfile_button_click() {
		if(this.selectedFiles.item(0) == null || this.selectedFiles.item(0) == undefined){
			swal.fire('Info','Please select file to upload','warning');
		}else if(this.selected_filecategory == null || this.selected_filecategory == undefined || this.selected_filecategory == ''){
			...
			if(this.selected_filecategory == 'ece'){
				...
				this.displayname = this.skillchartfilename_pgemath+'.'+this.filetype;
			}else if(this.selected_filecategory == 'pgeeng'){
				this.displayname = this.skillchartfilename_pgeeng+'.'+this.filetype;
			}else if(this.selected_filecategory == 'pgeodia'){
				this.displayname = this.skillchartfilename_pgeodia+'.'+this.filetype;
			}
			this.savetos3(this.currentFileUpload, this.displayname, this.selected_filecategory);
		}
	}
	...
	saveupdatetodb(filecategory, fileurl, displayname){
		if(filecategory == 'ece'){
			if(this.saveaction_ece == 'save') this.savetodb(filecategory, fileurl, displayname);
			else if(this.saveaction_ece == 'update') this.updatetodb(this.documentid_ece, filecategory, fileurl, displayname);
		}else if(filecategory == 'pgemath'){
			if(this.saveaction_pgemath == 'save') this.savetodb(filecategory, fileurl, displayname);
			else if(this.saveaction_pgemath == 'update') this.updatetodb(this.documentid_pgemath, filecategory, fileurl, displayname);
		}else if(filecategory == 'pgeeng'){
			if(this.saveaction_pgeeng == 'save') this.savetodb(filecategory, fileurl, displayname);
			else if(this.saveaction_pgeeng == 'update') this.updatetodb(this.documentid_pgeeng, filecategory, fileurl, displayname);
		}else if(filecategory == 'pgeodia'){
			if(this.saveaction_pgeodia == 'save') this.savetodb(filecategory, fileurl, displayname);
			else if(this.saveaction_pgeodia == 'update') this.updatetodb(this.documentid_pgeodia, filecategory, fileurl, displayname);
		}
	}
	...
	getdateuploaded(){
		...
		this.dateuploaded = (currentdate.getDate())+' - '+(currentdate.getMonth()+1)+' - '+(currentdate.getFullYear())+'   '+(currentdate.getHours())+' : '+(currentdate.getMinutes());
	}
