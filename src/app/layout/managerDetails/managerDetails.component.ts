...
export class ManagerDetailsComponent implements OnInit {
  ...
  getManagersDetails() {
    ...
    this.ManagerDetailsService.getAllManagersDetails(data).subscribe(
      (data: any) => {
  ...
  getBlockDetails() {
    this.ManagerDetailsService.getBlocks().subscribe(
      (data) => {
        ...
        for (var i = 0; i < this.allDisticBlocks.length; i++) {
          if (this.allDistics.length > 0) {
            ...
            for (var j = 0; j < this.allDistics.length; j++) {
              if (
                this.allDistics[j].district == this.allDisticBlocks[i].district
              ) {
                isDistic = true;
              }
            }
  ...
  show_picture(content, user) {
    ...
    this.ManagerDetailsService.getfeedbackimagebyid(id).subscribe(
      (data: any) => {
          ...
          if (
            ...
            this.image_preview =
              this.domSanitizer.bypassSecurityTrustResourceUrl(
                "data:image/jpg;base64," + data.images[0]
              );
          }
        } else {
          this.image_preview = null;
