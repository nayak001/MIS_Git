...
export class TrainingMiscComponent implements OnInit {
  ...
  addfile() {
    if (this.selectedFiles == undefined || this.selectedFiles == null) {
      ...
      for (let i = 0; i < this.selectedFiles.length; i++) {
        ...
        this.TrainingmiscService.pushFileToStorage(
  ...
  async save_record(body) {
    this.TrainingmiscService.savefiles(body).subscribe(
      (data) => {
        swal.fire(
          "Success",
  ...
  async update_record(id, body) {
    this.TrainingmiscService.updatefile(id, body).subscribe(
      (data) => {
  ...
  getallfiles() {
    this.hideLoading_indicator = false;
    this.TrainingmiscService.getallfiles().subscribe(
      (data) => {
        this.data = data;
  ...
  delete_data() {
    this.hideLoading_indicator = false;
    this.TrainingmiscService.deletefiles(this.file_id).subscribe(
      (data) => {
