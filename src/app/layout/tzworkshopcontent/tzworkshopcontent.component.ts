...
export class TzworkshopcontentComponent implements OnInit {
  ...
  get_ws_levels(wspreflang, wstype, wssubject, wsday) {
    ...
    this.tzworkshopcontentService
      .getwslevel(wspreflang, wstype, wssubject, wsday)
      .subscribe(
        (data) => {
  ...
  preferredlanguage_onchange(event: Event) {
    ...
    if (this.selected_action == "ip") {
      ...
      this.get_ws_levels(
        this.selected_preferredlanguage,
        this.selected_workshoptype,
        this.selected_subject,
        this.selected_workshopday
      );
  ...
  show_data() {
    if (
      this.selected_preferredlanguage == undefined ||
      this.selected_preferredlanguage == null ||
      this.selected_preferredlanguage == "" ||
      this.selected_workshopday == undefined ||
      this.selected_workshopday == null ||
      this.selected_workshopday == "" ||
      this.selected_workshoptype == undefined ||
      this.selected_workshoptype == null ||
      this.selected_workshoptype == "" ||
  ...
  add_new_content() {
    ...
    this.hideLoading_indicator = false;
    this.tzworkshopcontentService.createwscontent(body).subscribe(
      (data) => {
        swal.fire("Save", "Content saved " + data["status"], "success");
        this.hideLoading_indicator = true;
  ...
  update_existing_content() {
    ...
    this.hideLoading_indicator = false;
    this.tzworkshopcontentService
      .updatewscontent(this.record_id, body)
      .subscribe(
        (data) => {
          swal.fire("Save", "Content saved " + data["status"], "success");
          this.hideLoading_indicator = true;
  ...
  delete_button_clicked() {
    if (
      ...
      swal.fire("Data insufficient", "Nothing to delete!", "warning");
    } else {
      swal
        .fire({
          title: "Are you sure?",
  ...
  delete_content() {
    this.hideLoading_indicator = false;
    this.tzworkshopcontentService.deletewscontent(this.record_id).subscribe(
      (data) => {
        swal.fire("Save", "Content deletion " + data["status"], "success");
        this.hideLoading_indicator = true;
        this.show_data();
      },
      (error) => {},
  ...
  async update_record(id, body) {
    this.tzworkshopcontentService.updatewscontent(id, body).subscribe(
      (data) => {
