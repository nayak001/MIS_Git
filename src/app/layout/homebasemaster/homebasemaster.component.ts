...
export class HomebaseMasterComponent implements OnInit {
  ...
  alldata: any;
  async load_record() {
    this.HomebaseService.getallteacherassesment(
  ...
  async load_activity_record() {
    this.HomebaseService.getactivitydocument("hbl").subscribe(
      (data) => {
        if (Object.keys(data).length > 0) {
  ...
  updatequiz() {
    ...
    this.modalReference.close();
    this.HomebaseService.updatehomebasedmasterdata(
      this.edit_ques_id,
      body
    ).subscribe(
      (data) => {
  ...
  delquiz() {
    ...
    this.HomebaseService.deleteassesment(this.delete_ques_id).subscribe(
      (data) => {
  ...
  deleteactivity() {
    ...
    // 	displayname : ''
    // }
    this.HomebaseService.deletecontent(this.delete_doc_id).subscribe(
      (data) => {
  ...
  async addquiz() {
    if (this.add_q_question == "") {
      ...
      this.HomebaseService.createhomebasemasterdata(body).subscribe(
        (data) => {
  ...
  filechooser_onchange(event) {
    if (event.target.files.length > 0) {
      ...
      this.HomebaseService.pushFileToStorage(
  ...
  uploadactivitydoc() {
    ...
    this.HomebaseService.saveactivitydocument(body).subscribe(
      (data) => {
        swal.fire("Success", "assesment created successfully", "success");
        this.load_record();
        this.load_activity_record();
