...
@Component({
  selector: 'app-postcallactivities',
  templateUrl: './postcallactivities.component.html',
  styleUrls: ['./postcallactivities.component.scss'],
  animations: [routerTransition()]
})
export class PostcallactivitiesComponent implements OnInit {
  ...
  constructor(
    ...
    public router: Router,
		private postcallactivitiesService: PostcallactivitiesService
	) {}
  ...
  // -------------------------- Save -------------------------------
  save_data(){
    if(this.preferedlanguage == null || this.preferedlanguage == undefined || this.preferedlanguage == '') {
  ...
  delete_data(row){
    swal.fire({
      ...
      if (result.value) {
        ...
        this.hideLoading_indicator = false;
        this.postcallactivitiesService.deletemasterpostcallactivity(this.record_id).subscribe(data => {
  ...
  // ---------------------------- Search Result ----------------------
	search(term: string) {
		if(!term) {
		  ...
		  this.allpostcallrecordslist = this.allpostcallrecordslist_bkp.filter(element => 
			  element.question.trim().toLowerCase().includes(term.trim().toLowerCase())
		  );
		}
	}
