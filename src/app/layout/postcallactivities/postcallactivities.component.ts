...
export class PostcallactivitiesComponent implements OnInit {
  ...
  getmasterpostcallactivities(preferedlanguage, program) {
    if (
      ...
      this.postcallactivitiesService
        .getmasterpostcallactivities(preferedlanguage, program)
        .subscribe(
          (data) => {
            if (
  ...
  save_data() {
    if (
      ...
      this.postcallactivitiesService.savemasterpostcallactivity(body).subscribe(
        (data2) => {
  ...
  update_data() {
    if (
      ...
      this.postcallactivitiesService
        .updatemasterpostcallactivity(this.record_id, body)
        .subscribe(
          (data2) => {
            this.modalReference.close();
    ...
    swal
      ...
      .then((result) => {
        if (result.value) {
          ...
          this.hideLoading_indicator = false;
          this.postcallactivitiesService
            .deletemasterpostcallactivity(this.record_id)
            .subscribe(
              (data) => {
