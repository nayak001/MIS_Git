...
export class DashboardslidermanagerComponent implements OnInit {
  ...
  getalldashboardslides() {
    ...
    this.dashboardslidermanagerService.getalldashboardslides().subscribe(
      (data) => {
        if (
          data == undefined ||
          data == null ||
  ...
  save_data() {
    if (
      this.base64str == null ||
      this.base64str == undefined ||
      this.base64str == ""
    ) {
      swal.fire("Info", "Select an image", "warning");
    } else if (
      !(
        this.filetype.toLocaleLowerCase() == "jpg" ||
        this.filetype.toLocaleLowerCase() == "jpeg" ||
        this.filetype.toLocaleLowerCase() == "png"
      )
    ) {
      swal.fire("Info", "Image is not valid", "warning");
    } else if (
      this.slider_type == null ||
      this.slider_type == undefined ||
      this.slider_type == ""
    ) {
      ...
      this.dashboardslidermanagerService.savedashboardslides(body).subscribe(
        (data2) => {
  ...
  delete_data(row) {
    swal
      ...
      .then((result) => {
        if (result.value) {
          this.hideLoading_indicator = false;
          this.dashboardslidermanagerService
            .deletedashboardslides(row._id)
            .subscribe(
              (data) => {
                swal.fire(
                  "Success",
  ...
  _handleReaderLoaded(readerEvt) {
    ...
    this.base64img = this.domSanitizer.bypassSecurityTrustUrl(
      "data:image/jpg;base64," + this.base64str
    );
  }
}
