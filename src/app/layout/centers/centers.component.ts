...
export class CentersComponent implements OnInit {
  ...
  pageload_data() {
    ...
    this.centersService.getallcreatedcenters().subscribe(
      (data) => {
    ...
    // get all states
    this.hideLoading_indicator = false;
    this.centersService.getallstates().subscribe(
      (data) => {
  ...
  load_districts_arr(statecode) {
    ...
    this.all_districts =
      state_arr == undefined || state_arr == null || state_arr.length <= 0
        ? []
        : state_arr[0].districts;
  }
  ...
  formSubmitAction(centersubmitaction) {
    ...
    this.selected_centername =
      ...
      this.selected_centername.trim() == ""
        ? ""
        : this.selected_centername.toLowerCase();
    ...
    if (this.selected_centername.trim() == "") {
      ...
      swal.fire(
        "Info",
        "Center name must be of 3 or more character length.",
        "warning"
      );
      ...
      swal.fire(
        "Info",
        "Block name must be of 3 or more character length.",
        "warning"
      );
    } else {
  ...
  savecenter(body) {
    this.reset_all();
    this.centersService.createnewcenter(body).subscribe(
      (data) => {
  ...
  updatecenter(documentid, body) {
    this.reset_all();
    this.centersService.updatecenter(documentid, body).subscribe(
      (data) => {
        this.modalReference.close();
        this.reset_all();
        this.pageload_data();
  ...
  open(content, center) {
    if (Object.keys(center).length > 0) {
      ...
      this.selected_centername =
        center.centername == undefined ||
        center.centername == null ||
        center.centername.trim() == ""
          ? ""
          : center.centername;
      this.selected_statecode =
        center.statecode == undefined ||
        center.statecode == null ||
        center.statecode.trim() == ""
          ? ""
          : center.statecode;
      this.selected_statevalue =
        center.statevalue == undefined ||
        center.statevalue == null ||
        center.statevalue.trim() == ""
          ? ""
          : center.statevalue;
      this.load_districts_arr(this.selected_statecode);
      this.selected_districtid =
        ...
        center.districtid.trim() == ""
          ? ""
          : center.districtid;
      this.selected_districtvalue =
        center.districtvalue == undefined ||
        center.districtvalue == null ||
        center.districtvalue.trim() == ""
          ? ""
          : center.districtvalue;
      this.selected_blockname =
        center.block == undefined ||
        center.block == null ||
        center.block.trim() == ""
          ? ""
          : center.block;
