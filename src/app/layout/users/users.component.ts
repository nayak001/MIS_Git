...
export class UsersComponent implements OnInit {
  ...
  getpasscodebymanagerid(){
    ...
    this.usersService.getallprimarypasscodes(null).subscribe(data => {
  ...
  setpasscodelist(){
    if(this.allpasscodedata.length > 0){
      if(this.selected_usertype == 'manager'){
        this.passcodedata = this.allpasscodedata.filter(elem => {return elem.userid == this.selected_userid})
      }else{
  ...
  getallschoollistbypasscode(){
    this.hideLoading_indicator2 = false;
    this.usersService.getallschoolsregistered().subscribe(data => {
  ...
  setschoollist(){
    if(this.allschooldata.length > 0){
      if(this.selected_usertype == 'manager'){
        ...
        let arr = this.allschooldata.filter(elem => {return elem.passcode == this.selected_passcode});
  ...
  getdistrictsofstate() {
		this.hideLoading_indicator = false;
		this.usersService.getdistrictsofstate(this.selected_stateid).subscribe(data => {
  ...
  getblocksofdistricts() {
		this.hideLoading_indicator = false;
		this.usersService.getblocksofdistricts(this.selected_stateid, this.selected_districtid).subscribe(data => {
  ...
  selected_passcode_onchange(event: Event) {
    ...
    this.setschoollist();
    let arr = this.passcodedata.filter(elem => elem.passcode == this.selected_passcode)
    if(arr.length > 0){
  ...
  create_user(){
    if(this.savemanager_validation()){
      this.hideLoading_indicator = false;
      this.usersService.getuserbyuserid(this.selected_userid.toLowerCase()).subscribe(data => {
        this.hideLoading_indicator = true;

        if(Object.keys(data).length <= 0) {
          this.usersService.createnewuser(this.data_to_db).subscribe(data => {
  ...
  update_user(){
    if(this.selected_usertype == 'manager'){
      if(this.editmanager_validation()){
        this.hideLoading_indicator = false;
        this.usersService.updateuser(this._id, this.data_to_db).subscribe(data => {
          this.hideLoading_indicator = true;
          this.modalReference.close();
          this.getallUsers();
          this.reset_defaults();
  ...
  private getDismissReason(reason: any): string {
	...
	set_values(user){
		this._id = (user._id == undefined || user._id == null || user._id.trim() == '') ? '' : user._id;
		this.selected_username = (user.username == undefined || user.username == null || user.username.trim() == '') ? '' : user.username.trim().toLowerCase();
		this.selected_userid = (user.userid == undefined || user.userid == null || user.userid.trim() == '') ? '' : user.userid.trim();
		this.selected_password = (user.password == undefined || user.password == null || user.password.trim() == '') ? '' : user.password;
		this.selected_passcode = (user.passcode == undefined || user.passcode == null || user.passcode.trim() == '') ? '' : user.passcode;
		this.selected_usertype = (user.usertype == undefined || user.usertype == null || user.usertype.trim() == '') ? '' : user.usertype.trim().toLowerCase();
		this.selected_gender = (user.gender == undefined || user.gender == null || user.gender.trim() == '') ? 'male' : user.gender.trim().toLowerCase();
		this.selected_emailid = (user.emailid == undefined || user.emailid == null || user.emailid.trim() == '') ? '' : user.emailid.trim();
		this.selected_phone = (user.contactnumber == undefined || user.contactnumber == null || user.contactnumber.trim() == '') ? '' : user.contactnumber.trim();
		this.selected_qualification = (user.qualification == undefined || user.qualification == null || user.qualification.trim() == '') ? '' : user.qualification.trim();
		this.selected_address = (user.permanentaddress == undefined || user.permanentaddress == null || user.permanentaddress.trim() == '') ? '' : user.permanentaddress.trim().toLowerCase();
		this.selected_status = (user.status == undefined || user.status == null || user.status.trim() == '') ? '' : user.status.trim().toLowerCase();
		this.selected_centerid = (user.centerid == undefined || user.centerid == null || user.centerid.trim() == '') ? '' : user.centerid.trim();
		this.selected_centername = (user.centername == undefined || user.centername == null || user.centername.trim() == '') ? '' : user.centername.trim().toLowerCase();
		this.selected_udisecode = (user.udisecode == undefined || user.udisecode == null || user.udisecode.trim() == '') ? '' : user.udisecode.trim().toLowerCase();
		this.selected_schoolname = (user.schoolname == undefined || user.schoolname == null || user.schoolname.trim() == '') ? '' : user.schoolname.trim().toLowerCase();
		this.selected_managerid = (user.managerid == undefined || user.managerid == null || user.managerid.trim() == '') ? '' : user.managerid.trim().toLowerCase();
		this.selected_managername = (user.managername == undefined || user.managername == null || user.managername.trim() == '') ? '' : user.managername.trim().toLowerCase();
		this.selected_managertype = (user.managertype == undefined || user.managertype == null || user.managertype.trim() == '') ? '' : user.managertype.trim().toLowerCase();
		this.selected_stateid = (user.stateid == undefined || user.stateid == null || user.stateid.trim() == '') ? '20' : user.stateid.trim();
		this.selected_statename = (user.statename == undefined || user.statename == null || user.statename.trim() == '') ? 'odisha' : user.statename.trim().toLowerCase();
		this.selected_districtid = (user.districtid == undefined || user.districtid == null || user.districtid.trim() == '') ? '' : user.districtid.trim();
		this.selected_districtname = (user.districtname == undefined || user.districtname == null || user.districtname.trim() == '') ? '' : user.districtname.trim().toLowerCase();
		this.selected_blockid = (user.blockid == undefined || user.blockid == null || user.blockid.trim() == '') ? '' : user.blockid.trim().toLowerCase();
		this.selected_blockname = (user.blockname == undefined || user.blockname == null || user.blockname.trim() == '') ? '' : user.blockname.trim().toLowerCase();
		this.selected_userimage = (user.image == undefined || user.image == null || user.image.trim() == '') ? '' : user.image.trim();
	}
  ...
  savemanager_validation(){
    ...
    if(this.selected_userid == undefined || this.selected_userid == null || this.selected_userid.trim() == ''){
      ...
      return false;
		...
		}else if(this.selected_username == undefined || this.selected_username == null || this.selected_username.trim() == ''){
      swal.fire('Info', 'Please enter User name', 'info');
      return false;
    }else if(this.selected_username.length < 4 || this.selected_username.length > 20){
      swal.fire('Info', 'User name must be between 5 to 20 characters', 'info');
      return false;
    }else if(this.selected_password == undefined || this.selected_password == null || this.selected_password.trim() == ''){
      swal.fire('Info', 'Please enter Password', 'info');
      return false;
    }else if(this.selected_password.length < 5 || this.selected_password.length > 15){
      swal.fire('Info', 'Password must be between 5 to 15 characters', 'info');
      return false;
    }else if(this.selected_phone == undefined || this.selected_phone == null || this.selected_phone.trim() == ''){
      ...
      return false;
    }else if(this.selected_managertype == undefined || this.selected_managertype == null || this.selected_managertype.trim() == ''){
  ...
  editmanager_validation(){
    let regex_phone = /^((\\+91-?)|0)?[0-9]{10}$/;
    if(this.selected_passcode == undefined || this.selected_passcode == null || this.selected_passcode.trim() == ''){
  ...
  edituser_validation(){
    let regex_phone = /^((\\+91-?)|0)?[0-9]{10}$/;
    if(this.selected_username == undefined || this.selected_username == null || this.selected_username.trim() == ''){
      swal.fire('Info', 'Please enter User name', 'info');
      return false;
    }else if(this.selected_username.length < 4 || this.selected_username.length > 20){
      swal.fire('Info', 'User name must be between 5 to 20 characters', 'info');
      return false;
    }else if(this.selected_usertype == undefined || this.selected_usertype == null || this.selected_usertype.trim() == ''){
